    ...
    try {
        ...
        for (let i = 6; i < jsonData.length; i++) {
            ...
            if (row && row[1]) {
                ...
                // === ROBUST COUNTING LOGIC ADDED HERE ===
                // Get all session data ('P' or 'A')
                const sessionData = row.slice(6).filter(x => x === 'P' || x === 'A');

                // Calculate present and absent counts directly from session data
                const calculatedPresent = sessionData.filter(status => status === 'P').length;
                const calculatedAbsent = sessionData.filter(status => status === 'A').length;
                // =========================================
                ...
                studentData[rollNo] = {
                    ...
                    section: row[3] || '',
                    totalAbsent: calculatedAbsent,   // Use the new calculated value
                    totalPresent: calculatedPresent, // Use the new calculated value
...
async function fetchGitHubDirectory() {
    const fileStructure = {
        "ACEL": ["ACEL  Attendance - A.xlsx", "ACEL  Attendance - B.xlsx"],
        "ADMA": ["ADMA Attendance.xlsx"],
        "AIMLB": ["AIMLB Attendance - A.xlsx", "AIMLB Attendance - B.xlsx"],
        "B2B": ["B2B Attendance - A.xlsx", "B2B Attendance - B.xlsx"],
        "BF": ["BF Attendance - A.xlsx", "BF Attendance - B.xlsx"],
        "BM": ["BM Attendance - A.xlsx", "BM Attendance - B.xlsx"],
        "BMOD": ["BMOD Attendance - A.xlsx", "BMOD Attendance - B.xlsx"],
        "CB": ["CB Attendance - A.xlsx", "CB Attendance - B.xlsx"],
        "CCIT": ["CCIT Attendance.xlsx"],
        "CEDA": ["CEDA Attendance.xlsx"],
        "CHW": ["CHW Attendance.xlsx"],
        "DIW": ["DIW Attendance.xlsx"],
        "DV": ["DV Attendance - A.xlsx", "DV Attendance - B.xlsx"],
        "EsIndia": ["EsIndia Attendance - A.xlsx", "EsIndia Attendance - B.xlsx"],
        "ExO": ["ExO Attendance.xlsx"],
        "FAMA": ["FAMA Attendance - A.xlsx", "FAMA Attendance - B.xlsx"],
        "FAuR": ["FAuR Attendance.xlsx"],
        "FBETDM": ["FBETDM Attendance.xlsx"],
        "FMac": ["FMac Attendance.xlsx"],
        "GS": ["GS Attendance.xlsx"],
        "IAPM": ["IAPM Attendance - A.xlsx", "IAPM Attendance - B.xlsx"],
        "IF": ["IF Attendance - A.xlsx", "IF Attendance - B.xlsx"],
        "ITA": ["ITA Attendance.xlsx"],
        "IoTB": ["IoTB Attendance.xlsx"],
        "LMPO": ["LMPO Attendance.xlsx"],
        "LMW": ["LMW Attendance - A.xlsx", "LMW Attendance - B.xlsx"],
        "LSS": ["LSS Attendance.xlsx"],
        "M&A": ["M&A Attendance.xlsx"],
        "MACR": ["MACR Attendance.xlsx"],
        "MLVW": ["MLVW Attendance.xlsx"],
        "MSN": ["MSN Attendance - A.xlsx", "MSN Attendance - B.xlsx"],
        "OCMC": ["OCMC Attendance.xlsx"],
        "OFDG": ["OFDG Attendance - A.xlsx", "OFDG Attendance - B.xlsx"],
        "PM": ["PM Attendance - A.xlsx", "PM Attendance - B.xlsx"],
        "People": ["People Attendance.xlsx"],
        "Pricing": ["Pricing Attendance - A.xlsx", "Pricing Attendance - B.xlsx"],
        "Python": ["Python Attendance - A.xlsx", "Python Attendance - B.xlsx"],
        "SA": ["SA Attendance.xlsx"],
        "SAR": ["SAR Attendance.xlsx"],
        "SM": ["SM Attendance - A.xlsx", "SM Attendance - B.xlsx"],
        "SS": ["SS Attendance - A.xlsx", "SS Attendance - B.xlsx", "SS Attendance - C.xlsx", "SS Attendance - D.xlsx", "SS Attendance - E.xlsx", "SS Attendance - F.xlsx", "SS Attendance - G.xlsx", "SS Attendance - H.xlsx", "SS Attendance - I.xlsx", "SS Attendance - J.xlsx"],
        "TOC": ["TOC Attendance.xlsx"]
    };
    ...
    try {
        ...
        await Promise.all(files.map(async (file) => {
            try {
                ...
                const response = await fetch(file.download_url, { cache: 'no-store' }); // Keep cache off
                if (!response.ok) {
